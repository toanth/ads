...
struct CacheEfficientLayout : DefaultLayoutImpl<CacheEfficientLayout> {

    std::unique_ptr<Elem[]> vec;

    explicit CacheEfficientLayout([[maybe_unused]] Index numElems) noexcept
...
struct SimpleLayout : DefaultLayoutImpl<SimpleLayout<SuperblockSize, BlockSize>> {
...
public:
    ...
    [[nodiscard]] Index completeSizeInElems() const noexcept {
        //        constexpr static Index blocksPerElem = 8 / blockSize();
        //        return roundUpDiv(this->numBlocks(), blocksPerElem) + numSuperblocks() * superblockSize() + numElems();
